...
namespace ru_football
{
    ...
    public class Calculator : ICalculator
    {
        ...
        public string CalculateTourResult(string numberString)
        {
            ...
            statistic += @"</tr>";
            statistic = AddStatisticRow(results, numbers, statistic, "Угаданных счетов<br/>(4 очка)",
                                        ScoreType.ScoreMatch);
            statistic = AddStatisticRow(results, numbers, statistic, "Угаданных разниц<br/>(2 очка)",
                                        ScoreType.Difference);
            statistic = AddStatisticRow(results, numbers, statistic, "Угаданных исходов<br/>(1 очко)", ScoreType.Result);
            statistic = AddStatisticRow(results, numbers, statistic, "Угаданно всего<br/>(хотя бы 1 очко)",
        ...
        private IEnumerable<IGrouping<Ljuser, Forecast>> OrderedByPercentLjusers()
        {
            ...
            using (unitOfWorkFactory.Create())
            {
                users = queryFactory.FindAll<Ljuser>().Execute().Where(x => x.Forecasts.Count() > 50).ToList();
                allForecasts = users.SelectMany(x => x.Forecasts).ToList();
                IEnumerable<Match> allMatches = queryFactory.FindAll<Match>().Execute().ToList();
